...
final class ThemeStore: ObservableObject {
    ...
    init() {
        ...
        let savedScale = ud.object(forKey: Keys.fontScale) as? Double
        self.fontScale = savedScale ?? 1.0
        
        let bgRaw = ud.object(forKey: Keys.backgroundStyle) as? Int ?? BackgroundStyle.system.rawValue
        self.backgroundStyle = BackgroundStyle(rawValue: bgRaw) ?? .system
        
        let presetRaw = ud.object(forKey: Keys.accentPreset) as? Int ?? AccentPreset.blue.rawValue
        self.accentPreset = AccentPreset(rawValue: presetRaw) ?? .blue
        
        self.customAccentHex = ud.string(forKey: Keys.customAccentHex) ?? ""
        self.useCustomAccent = ud.object(forKey: Keys.useCustomAccent) as? Bool ?? false
        
        let appRaw = ud.object(forKey: Keys.appearance) as? Int ?? AppAppearance.system.rawValue
    ...
    func setFontScale(_ scale: Double) {
        fontScale = scale
        UserDefaults.standard.set(scale, forKey: Keys.fontScale)
    }
    
    func setBackgroundStyle(_ style: BackgroundStyle) {
        backgroundStyle = style
        UserDefaults.standard.set(style.rawValue, forKey: Keys.backgroundStyle)
    }
    
    func setAccentPreset(_ preset: AccentPreset) {
        accentPreset = preset
        useCustomAccent = false
        UserDefaults.standard.set(preset.rawValue, forKey: Keys.accentPreset)
        UserDefaults.standard.set(false, forKey: Keys.useCustomAccent)
    }
    
    func setUseCustomAccent(_ enabled: Bool) {
        useCustomAccent = enabled
        UserDefaults.standard.set(enabled, forKey: Keys.useCustomAccent)
    }
    
    /// 允許輸入 "#FF8800" / "FF8800" / "fff" 這三種
    func setCustomAccentHex(_ hex: String) {
        customAccentHex = hex
        UserDefaults.standard.set(hex, forKey: Keys.customAccentHex)
        
        if Color(hex: hex) != nil {
            useCustomAccent = true
            UserDefaults.standard.set(true, forKey: Keys.useCustomAccent)
        }
    }
    
    func setAppearance(_ a: AppAppearance) {
        appearance = a
        UserDefaults.standard.set(a.rawValue, forKey: Keys.appearance)
    }
