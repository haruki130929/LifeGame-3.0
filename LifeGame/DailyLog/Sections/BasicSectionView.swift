...
struct BasicSectionView: View {
    ...
    var header: String = String(localized: "基本")
    ...
    var body: some View {
        Section(header) {
            ...
            // 起床時間：可忘記 + 30 分鐘刻度
            HalfHourOptionalTimePickerRow(title: String(localized: "起床時間"), value: $entry.wakeTime)

            // 上床時間：30 分鐘刻度
            HalfHourOptionalTimePickerRow(title: String(localized: "昨晚上床時間"), value: $entry.bedTime)
        }
    }
}
