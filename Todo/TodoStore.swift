...
final class TodoQuadrantStore: ObservableObject {
    ...
    private let filename = "todo_quadrant_v1.json"
    ...
    private func load() {
        // ✅ 先嘗試從檔案讀（新版）
        let box: VersionedBox<[TodoItem]> = SafeLoad.loadOrDefault(
            store: fileStore,
            filename: filename,
            defaultValue: VersionedBox(version: 1, payload: [])
        )
        items = box.payload
    ...
    private func save() {
        do {
            let box = VersionedBox(version: 1, payload: items)
            try fileStore.save(box, to: filename)
        } catch {
            print("TodoQuadrantStore save failed:", error)
        }
    }
}
