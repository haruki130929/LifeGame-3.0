...
private struct HomeContentView: View {
    ...
    @State private var isKyudoFormPresented = false
...
// MARK: - FAB Actions
private extension HomeContentView {
    func setupFab(for slot: TimeSlot) {
        // ✅ 你可以依時段改標題/內容；先給一個好懂的版本
        let slotName = slot.rawValue  // 你現在 rawValue 是 "morning" 那種，也可自己改成中文
        
        fab.setRootActions([
            FabAction(title: "\(slotName)－新增弓道筆記", systemImage: "book") {
                print("FAB: KyudoNote for \(slotName)")
                // 之後你想開表單，就在這裡改成 isKyudoFormPresented = true
            },
            FabAction(title: "\(slotName)－每日紀錄", systemImage: "square.and.pencil") {
                print("FAB: DailyLog for \(slotName)")
            },
    ...
    @ViewBuilder private var contentLinks: some View {
        ...
        case .diary:
            NavigationLink(destination: DiaryView()) {
                Label("寫日記", systemImage: "square.and.pencil")
            }
            
            NavigationLink(destination: KyudoNoteView()) {
